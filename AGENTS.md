...
13. **Prima di pubblicare `firestore.rules`/`storage.rules` (qualunque `firebase deploy` o strumento MCP equivalente), verifica sempre di essere sul progetto Firebase giusto — non darlo mai per scontato.** Il 25/09/2026 l'ambiente del server MCP Firebase risultava puntato sul progetto sbagliato (`thats-a-blast-app`) invece di `egadi-sailing-2026`, nonostante la working directory contenesse il `.firebaserc` corretto: un primo deploy fatto in quello stato ha riportato "successo" ma non aveva realmente aggiornato le regole del progetto giusto (rilettura post-deploy che mostrava ancora il contenuto vecchio). Prima di ogni pubblicazione dell'agente:
    1. Controlla l'ambiente attivo (es. `firebase_get_environment` per l'MCP, oppure `firebase use` da CLI) e conferma che il progetto attivo corrisponda all'alias `default` di `.firebaserc` (`egadi-sailing-2026`). Se non corrisponde, correggilo esplicitamente prima di procedere (es. `firebase_update_environment` con `active_project`, o `firebase use egadi-sailing-2026` da CLI) — non assumere che coincida solo perché la working directory è quella giusta.
    2. Dopo il deploy, **rileggi le regole effettivamente pubblicate** (es. `firebase_get_security_rules` o `firebase deploy` via CLI, che compila e mostra gli avvisi prima di rilasciare) e cerca un frammento noto della modifica appena fatta, invece di fidarti solo del messaggio "successo" del comando: quello può riferirsi al progetto sbagliato.
    3. Se un tool MCP e il CLI `firebase` danno risultati diversi per la stessa operazione, fidati del CLI (più diretto, meno stato nascosto) e segnala la discrepanza invece di ignorarla.
