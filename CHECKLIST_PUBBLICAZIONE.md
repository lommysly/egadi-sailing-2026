...
- [x] Avvio area skipper: il controllo sulle quote gestisce anche l’assenza di una scheda aperta, così non interrompe il pulsante di accesso Google. Il test locale `node --test tools/area-projection-guards.test.mjs` blocca la stessa regressione prima del deploy; resta obbligatorio aprire l’area anonima e controllare la console prima di ogni pubblicazione di `area.js`.
- [x] Accesso Google resistente ai browser mobili: su iPhone e Android usa il reindirizzamento anziché una finestra popup; su desktop, se la finestra Google viene chiusa, appare il recupero «Accedi senza popup» che riporta automaticamente all’area skipper.
...
- [x] Confronto diretto Firestore–coda transfer–Google Sheet per Karibu (23/09/2026): dieci tratte attive nel backup, cinque richieste con consenso nella coda; le altre restano fuori dalla coda correttamente. Corrette le diciture che confondevano viaggio salvato e transfer scelto; il foglio non deve più chiamare «Revocato» una tratta attiva senza transfer.
- [x] Verificato sul sito pubblicato (23/09/2026) il ritorno diretto da `transfer.html` all'area skipper con lo stesso account, senza nuovo login; la coda ora mostra anche il rientro di Roberta dopo la sua scelta e il suo consenso. Verificato nel foglio Arrivi/Partenze che le quattro tratte senza scelta mostrino «Transfer da scegliere» senza nome o contatto. La vecchia scheda Movimenti è stata conservata come «Archivio (non aggiornato)» e il file Google è stato ristretto al proprietario e all'account tecnico, rimuovendo l'accesso «chiunque abbia il link».
- [x] Corretto il riepilogo della scheda viaggio: alla riapertura legge andata e rientro direttamente dal server, mostra in evidenza l'ultimo stato salvato e chiama il pulsante «Salva modifiche» se la tratta è già confermata; in questo caso non offre più «Salva bozza», che poteva riportarla involontariamente allo stato da confermare. Gli aggiornamenti in diretta riallineano i campi soltanto se la persona non sta compilando una modifica; il test sul dispositivo della partecipante resta separato.
- [x] Aggiunto un cambio area persistente nel portale transfer per chi usa lo stesso account come skipper e gestore. Nel documento minimo `crewTravelStatus` lo stato operativo è proiettato dal server, leggibile solo dalla persona interessata e mostrato con icone sia in dashboard sia nella scheda di viaggio. Il riepilogo skipper usa gli stessi stati: una richiesta appena salvata non appare più come transfer confermato. Verificate nell'emulatore le transizioni operative `planned` → `confirmed`, la lettura puntuale propria e il diniego di lista, lettura altrui, scrittura client e accesso dopo cambio briefing; resta da provare sul dispositivo di una partecipante la transizione reale «in organizzazione» → «confermato».
...
- [ ] Test bozze private con soli dati fittizi: salvare skipper e crew con campi lasciati intenzionalmente vuoti, rientrare e verificare il precompilamento; controllare che i record `skipperProfileDraft/default` e `crewDrafts/{inviteId}` siano inaccessibili agli altri ruoli, che il PDF resti invariato e che il salvataggio finale promuova i dati completi cancellando la sola bozza del titolare/versione attivi.
- [ ] Test viaggio privato skipper con soli dati fittizi: salvare `skipperTravel/outbound` e `skipperTravel/return` separatamente, anche con campi vuoti, rientrare e verificare il precompilamento. Provare transfer attivo solo con volo e aeroporto `TPS`/`PMO`; verificare che organizzatore, equipaggio, altra barca, società transfer, estraneo, `list` e delete siano negati e che i dati non compaiano in Crew List, PDF, flotta, richieste contributo o contabilità.
...
- [ ] Test contributi: ogni nuova richiesta usa soltanto `cost_recovery` o `other`; Starter Pack, lenzuola/asciugamani e cauzione rimborsabile non compaiono nelle richieste WhatsApp. Una richiesta legacy senza categoria resta verificabile o annullabile senza poterne riscrivere il contenuto. Verificare inoltre profilo incasso privato con uno o più metodi e relativi dettagli, WhatsApp con solo i dettagli dei metodi selezionati, destinatario con briefing corrente vede solo le proprie richieste e soltanto lo skipper registra “verificato” o “annullata” dopo controllo esterno reale; organizzatore e altri skipper sono negati. La nota libera deve restare nel figlio privato dello skipper, ricomparire identica in “Apri WhatsApp” e “Copia messaggio” e restare negata a equipaggio, organizzatore, altri skipper e `list`. Su Mac verificare l'apertura nell'app, con “Apri WhatsApp nel browser” come recupero. Un click non prova il pagamento.
- [ ] Test dichiarazione di pagamento (riconciliazione da `codex/partial-profile-saves`, 22/09/2026): con soli dati fittizi, il destinatario di una richiesta `prepared` dichiara "Ho pagato" scegliendo uno dei soli metodi proposti in quella richiesta; verificare che compaia una sola volta (il controllo scompare dopo), che lo skipper veda l'avviso "La persona dichiara di aver pagato..." nella propria lista senza che lo stato cambi da solo, e che "Conferma accredito" resti l'unica azione che porta a `verified`. Provare a dichiarare una richiesta altrui, una già dichiarata, una `manual_receipt`, una già `verified`/`cancelled`, e un metodo non proposto: devono essere tutti negati. Vedi caso "Dichiarazione di pagamento" in `FIRESTORE_RULES_TEST_MATRIX.md`.
...
- [ ] Definire data e responsabile per cancellare Crew List, bozze `crewDrafts`/`skipperProfileDraft`, inviti, `crewAccess`, `crewLoginIndex`, `costPlan/default`, le due copie private Storage, account Firebase tecnici, PDF locali e messaggi WhatsApp dopo l'evento; Cassa skipper, dossier e Storage richiedono una procedura amministrativa perché l'area skipper non può eliminarli.
