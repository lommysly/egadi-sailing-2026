...
2. Quando decide di coinvolgere quella persona, lo skipper trasforma la stessa proiezione in un invito: il sito mantiene il medesimo identificativo e genera un link personale casuale, valido 14 giorni. Lo skipper lo invia direttamente su WhatsApp: è l’unico invito e riepiloga sistemazione, quota da versare (posto + assicurazione), Starter Pack cash con le voci comprese e cauzione rimborsabile cash. L’invito fotografa importi e voci del Pack nella dashboard personale, ma non è un incasso: fino a un acconto o saldo della quota verificato, lo skipper può correggere dalla stessa scheda ruolo, partecipazione alle quote, sistemazione, cabina e importi. Dopo la verifica, questi elementi restano protetti. La dashboard personale ripete il riepilogo e mostra i metodi di versamento che lo skipper ha abilitato, con causale copiabile. Il versamento avviene fuori dal sito e viene controllato manualmente dallo skipper. Messaggio, Privacy e primo accesso usano la lingua selezionata.
...
Nel profilo privato dello skipper possono essere conservati solo i dettagli necessari per ricevere il contributo: link HTTPS PayPal/Satispay, link HTTPS o Revtag Revolut e, per bonifico, IBAN più intestatario. Non inserire mai password, OTP, numeri di carta, chiavi API o credenziali dei provider. I dettagli non entrano nella Crew List o nei dati pubblici: una copia ridotta e coerente con i soli metodi abilitati è leggibile nella dashboard del partecipante solo dopo l’accettazione del briefing corrente. WhatsApp può restare un promemoria, ma non è l’unico modo per recuperare le istruzioni.
...
Il **Piano equipaggio** serve allo skipper per fare una previsione puntuale prima di inviare WhatsApp: a ogni posto riservato associa persona, ruolo, sistemazione e quattro componenti personali — posto, Starter Pack, assicurazione cauzione e cauzione rimborsabile. Il posto può essere una cabina oppure, in alternativa, la dinette. La **richiesta prevista** somma posto e assicurazione. Lo Starter Pack può essere già compreso nel charter oppure essere un costo esterno; in entrambi i casi resta una quota in contanti in loco. Quando è già compreso, il calcolatore ne scorpora il valore dalla quota cabina. Per co-skipper, hostess o collaboratori gratuiti lo skipper può disattivare la partecipazione alle quote: il posto resta nel piano e nella capienza, mentre quota posto, Starter Pack e assicurazione restano a zero. La cauzione rimborsabile rimane distinta e lo skipper può comunque prevederla. Fino a un contributo della quota verificato, la stessa scheda permette di correggere ruolo, partecipazione alle quote, sistemazione, cabina e importi; dopo la verifica, il sito li protegge per evitare discrepanze con acconti e saldi. Nome, WhatsApp e link non cambiano nella modifica ordinaria perché sono anche credenziali dell’accesso personale. Tutte sono previsioni utili a stimare la copertura dei costi e vengono mostrate nella dashboard personale dopo l'attivazione, ma non sono una richiesta, un incasso, un pagamento verificato o un impegno automatico.
...
Il sito non incassa denaro, non genera o valida link dei provider e non dichiara pagamenti come eseguiti. Lo skipper configura una volta il proprio nome, i metodi e i dettagli privati di PayPal, Satispay, Revolut e/o bonifico; dopo il briefing corrente la persona ritrova nella propria dashboard solo i metodi abilitati, le coordinate necessarie e una causale copiabile. Lo skipper può anche creare una richiesta con importo, causale, scadenza e una o più alternative per un promemoria, un saldo aggiornato o un extra. Ogni nuova richiesta porta due etichette tecniche chiuse: `cost_recovery` se il versamento deve concorrere al recupero dei costi della barca, `other` negli altri casi; e `contributionItemId`, che identifica quota posto, assicurazione o altra voce senza interpretare la causale libera. Starter Pack e cauzione rimborsabile non sono ammessi nelle nuove richieste. Lo Starter Pack resta sempre cash/in loco; il flag “compreso nel charter” indica solo che il suo valore è già dentro il charter e viene scorporato dalla quota cabina. La cauzione resta sempre cash/in loco. Un'eventuale nota libera del messaggio vive nel figlio privato e immutabile `paymentRequests/{requestId}/private/message`: serve allo skipper per riaprire o copiare lo stesso testo, ma non compare nell'area della persona. Su Mac il pulsante prova l'app WhatsApp nativa; il recupero esplicito resta WhatsApp nel browser o la copia del messaggio. Il pagamento avviene fuori dal sito e può essere segnato come verificato solo dallo skipper, dopo controllo manuale dell'accredito reale. Le richieste create prima dell'introduzione di queste etichette restano aggiornabili soltanto nelle normali transizioni di stato, così possono essere verificate o annullate senza riscriverne il contenuto; restano fuori dai riepiloghi classificati finché il sito non conosce la loro voce.

Non tutti versano dopo una richiesta nominale: chi paga di propria iniziativa (24/09/2026, caso reale di Carla Olivieri) trova nella propria dashboard, sotto i metodi pubblicati, un modulo per segnalare subito importo e metodo usato. La segnalazione crea una richiesta già "dichiarata" a se stessa (`entryType: "self_reported"`), senza collegamento contabile né dati riservati allo skipper: lo skipper la vede e la verifica con lo stesso pulsante "Conferma accredito" di ogni altra richiesta.
...
La sezione pubblica descrive il flusso per aeroporto, Marsala e passaggi fra amici. Equipaggio e skipper compilano andata e rientro separatamente nelle rispettive aree private (`travel.html` per l'equipaggio, `skipperTravel` per lo skipper); i dati non entrano nella Crew List, nel PDF, nella flotta o nella contabilità. Le bozze incomplete restano salvabili.

Nel modulo equipaggio, la conferma di un volo richiede una scelta esplicita per il collegamento aeroporto ↔ Marsala: transfer organizzato oppure autonomia. Se la persona non lo sa ancora, salva una bozza. Per chiedere il transfer deve inoltre dare il consenso dedicato. La card distingue viaggio confermato, transfer richiesto e collegamento non indicato; i pulsanti e i messaggi di esito nominano sempre andata o rientro. Alla riapertura le tratte vengono lette dal server, l'ultimo stato salvato è mostrato in evidenza e una tratta già confermata offre «Salva modifiche» anziché chiedere di confermarla di nuovo; «Salva bozza» viene nascosto per evitare di riportarla involontariamente a uno stato incompleto. Un aggiornamento in diretta riallinea i campi soltanto se la persona non ha modifiche non salvate. Una bozza con transfer e consenso è visibile al coordinatore solo quando è indicato anche l'aeroporto Trapani (TPS) o Palermo (PMO), come previsto dal processo server. Il test locale mobile in Chromium e WebKit ha verificato che scelta e consenso restano nel form anche passando a un'altra scheda; resta separata la prova autenticata dal dispositivo della partecipante.
...
Il riepilogo skipper delle tratte legge `crewTravelStatus`: conta anche un viaggio già salvato se la scelta del collegamento aeroporto–Marsala è ancora vuota. La coda della società transfer, invece, mostra esclusivamente le richieste con consenso esplicito; la differenza fra i due conteggi non indica da sola una perdita di dati. Nel riepilogo skipper «Transfer richiesto» non significa ancora confermato: il badge segue poi «In organizzazione», «Transfer confermato», «Transfer concluso» o «Transfer annullato» dallo stesso stato operativo che vede la persona. Chi è contemporaneamente skipper e referente transfer trova nel portale un cambio area persistente per tornare direttamente alla propria barca senza uscire, purché usi lo stesso account Google; un account operativo diverso non diventa skipper. Il foglio di backup registra anche le tratte senza richiesta, oscurando nome e contatto quando il consenso transfer manca; in questo caso indica «Transfer da scegliere» o «Transfer non richiesto», mai «Revocato» per un viaggio tuttora attivo.

Lo stesso documento `crewTravelStatus` contiene ora soltanto lo stato operativo minimo di andata e rientro (`new`, `planned`, `confirmed`, `completed`, `cancelled`): la Cloud Function lo aggiorna quando cambia la scheda del gestore transfer, mantenendo una revisione per evitare che eventi vecchi prevalgano. La persona legge puntualmente solo il proprio documento dopo briefing corrente, senza nomi o recapiti di altri. Nella propria dashboard e nella scheda Arrivi e partenze vede con icone la differenza fra bozza, richiesta salvata, pianificazione, conferma, conclusione o annullamento. La conferma del viaggio da parte della persona non equivale mai a conferma del transfer da parte del gestore; nessun dettaglio di ritrovo viene esposto automaticamente.
