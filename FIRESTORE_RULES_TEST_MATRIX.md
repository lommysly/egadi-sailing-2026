...
- `TRANSFER_A`: tratta fittizia in `events/egadi-2026/transferProfiles/CREW_A/transferLegs/{id}` con `ownerUid: CREW_A`, `boatId: SKIPPER_A`, `inviteId: INVITE_A`, città e aeroporto del catalogo, data fittizia, `matchTime: "15:30"`, campi bagagli coerenti e soli timestamp server. Non usare volo, telefono, auto o dati reali.
...
| Tratta personale | Dopo claim, `CREW_A` crea, legge, aggiorna ed elimina solo `TRANSFER_A` nel proprio percorso `transferProfiles/CREW_A/transferLegs`. Prova `ownerUid`, barca, invito, `createdAt` o un campo extra diversi; prova orario non `HH:mm`, aeroporto non IATA, posto auto senza veicolo, più di cinque posti o consenso operatore su una tratta città ↔ aeroporto. | Consenti soltanto lo schema coerente e i campi ammessi; nega tutte le varianti. |
| Isolamento transfer | `CREW_B`, skipper, outsider e anonimo provano `get` e `list` su `TRANSFER_A`; `CREW_A` prova un percorso `transferProfiles/CREW_B/transferLegs`. L'organizzatore legge la tratta fittizia. | Solo `CREW_A` nel proprio percorso e l'organizzatore possono leggere. Skipper e altri equipaggi non vedono tratte né contatti. |
...
6. Nella mia area, creare e modificare una tratta fittizia: Torino → Malpensa (MXP), ora `15:30`, compagnia dal catalogo e poi “offro posti in auto a noleggio”. Verificare selezioni guidate, campo “Altra città”, orari a 24 ore, salvataggio e cancellazione; nessun elenco deve mostrare telefoni o tratte di altri equipaggi.
